...
export function InventoryRecord() {
  ...
  const handleWithdrawalItemSelect = (itemName: string) => {
    ...
    if (isEditWithdrawalModalOpen && withdrawalForm.dateOfWithdrawal) {
      ...
      setWithdrawalForm({ ...withdrawalForm, itemName });
    }
  };
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="mb-6">
              <TabsTrigger value="purchase">Inventory Purchase Register</TabsTrigger>
              <TabsTrigger value="withdrawal">Inventory Withdrawal Register</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="purchase">
              <div className="space-y-4">
                <div className="flex justify-end gap-2">
                  <Button
                    variant="outline"
                    onClick={() => setShowAllPurchaseRecords(!showAllPurchaseRecords)}
                    className="flex items-center gap-2"
                  >
                    <Eye className="w-4 h-4" />
                    {showAllPurchaseRecords ? 'Show Today Only' : 'View All Records'}
                  </Button>
                  <Dialog open={isPurchaseModalOpen} onOpenChange={(open: boolean) => {
                    ...
                    <DialogContent className="max-w-2xl">
                      <DialogHeader>
                        <DialogTitle>Add Inventory Purchase Record</DialogTitle>
                      </DialogHeader>
                      <div className="grid grid-cols-2 gap-4 py-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            type="number"
                            placeholder="e.g., 500"
                            value={purchaseForm.currentStock}
                            onChange={(e) => setPurchaseForm({ ...purchaseForm, currentStock: e.target.value })}
                          />
                        </div>
                      </div>
                ...
                <Dialog open={isEditPurchaseModalOpen} onOpenChange={(open: boolean) => {
                  ...
                  <DialogContent className="max-w-2xl">
                    <DialogHeader>
                      <DialogTitle>Edit Inventory Purchase Record</DialogTitle>
                    </DialogHeader>
                    <div className="grid grid-cols-2 gap-4 py-4">
                      <div className="space-y-2">
                        <Label>Date of Purchase</Label>
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          value={purchaseForm.quantityPurchased}
                          onChange={(e) => setPurchaseForm({ ...purchaseForm, quantityPurchased: e.target.value })}
                          disabled={!editingPurchaseId} // Disable if no specific record is selected for editing
                        />
                      </div>
                      <div className="space-y-2">
                        <Label>Current Stock</Label>
                    ...
                    <div className="flex justify-between gap-3">
                      <Button
                        ...
                        onClick={() => {
                          if (editingPurchaseId) {
                            setItemToDelete({ type: 'purchase', id: editingPurchaseId });
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs text-gray-600">Date of Purchase</th>
                        <th className="px-4 py-3 text-left text-xs text-gray-600">Item Name</th>
                        <th className="px-4 py-3 text-left text-xs text-gray-600">Quantity Purchased</th>
                        <th className="px-4 py-3 text-left text-xs text-gray-600">Current Stock</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredPurchaseData.map((record) => (
                        <tr key={record.id} className="border-b hover:bg-gray-50">
                          <td className="px-4 py-3 text-sm">{record.id}</td>
                          <td className="px-4 py-3 text-sm">{record.dateOfPurchase}</td>
                          <td className="px-4 py-3 text-sm">
                            <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                              {record.itemName}
                            </Badge>
                          </td>
                          <td className="px-4 py-3 text-sm">{record.quantityPurchased}</td>
            ...
            <TabsContent value="withdrawal">
              <div className="space-y-4">
                <div className="flex justify-end gap-2">
                  <Button
                    variant="outline"
                    onClick={() => setShowAllWithdrawalRecords(!showAllWithdrawalRecords)}
                    className="flex items-center gap-2"
                  >
                    <Eye className="w-4 h-4" />
                    {showAllWithdrawalRecords ? 'Show Today Only' : 'View All Records'}
                  </Button>
